...
import { getMetrics, deleteMetricById } from 'apiSdk/metrics';
import { MetricInterface } from 'interfaces/metric';

type ColumnType = ColumnDef<MetricInterface, unknown>;
...
export function MetricListPage(props: MetricListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MetricInterface>>(
    () => `/metrics?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MetricInterface) => {
    if (hasAccess('metric', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/metrics/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/metrics/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('metric', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
          ...
          {hasAccess('metric', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="metric" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Metrics
        </Text>
        {hasAccess('metric', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
