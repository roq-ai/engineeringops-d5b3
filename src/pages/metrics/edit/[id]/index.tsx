...
import { getMetricById, updateMetricById } from 'apiSdk/metrics';
...
function MetricEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<MetricInterface>(
    () => (id ? `/metrics/${id}` : null),
    () => getMetricById(id),
  );
  ...
  const handleSubmit = async (values: MetricInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateMetricById(id, values);
      mutate(updated);
      resetForm();
      router.push('/metrics');
    } catch (error) {
